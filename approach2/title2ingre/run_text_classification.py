...
# os.environ["WANDB_PROJECT"] = "<task1_data1_mbert_uncased>"  # name your W&B project
...
class DataTrainingArguments:
    ...
    metric_name: Union[Optional[str], List[str]] = field(default=None, metadata={"help": "The metric to use for evaluation."})
    # train_file: local dataset file path
    ...
    is_single_classification: bool = field(
        default=False, metadata={"help": "Whether to do single classification."}
    )
...
class ModelArguments:
    ...
    #! add this parameter when we want to use a different config 
    config_name: Optional[str] = field(
        default=None, metadata={"help": "Pretrained config name or path if not the same as model_name"}
    )
    #! add this parameter when we want to use a different tokenizer
    ...
    early_stopping_patience: int = field(
        default=3,
        metadata={"help": "The number of evaluation calls to wait before stopping training."},
    )
...
# # ! Preprocess data set
def preprocess_label_list_multi_label(example):
    """Map to the rawdataset before we load dataset as train/eval/predict"""

    return example

def preprocess_label_list_multi_class(example):
    """Map to the rawdataset before we load dataset as train/eval/predict"""
...
def main():
    ...
    if len(sys.argv) == 2 and sys.argv[1].endswith(".json"):
        ...
        model_args, data_args, training_args = parser.parse_args_into_dataclasses() # 用于添加parser的参数
    ...
    if data_args.dataset_name is not None:
        ...
        #!: Preprocess data set 
        if data_args.is_single_classification:
            raw_datasets = raw_datasets.map(preprocess_label_list_multi_class)
        else:
            raw_datasets = raw_datasets.map(preprocess_label_list_multi_label) 
    ...
    # This is one-line if-else statement, when label column is "float32", "float64", it will do regression automatically
    is_regression = (
    ...
    if is_regression:
        ...
        for split in ["validation", "test"]:
            if split in raw_datasets:
                ...
                # find the labels that appear in val/test but not in train
    ...
    if is_regression:
        ...
        logger.info(f"setting problem type to {config.problem_type}")
    elif is_multi_label:
    ...
    def multi_labels_to_ids(labels: List[str]) -> List[float]:
        ...
        return ids
    #! preprocess function in Trainer
    def preprocess_function(examples): # example is a row of raw_datasets
        """Concatenate all input columns into one sentence column, tokenize, pad and truncate the sentence"""
        if data_args.text_column_names is not None:
            text_column_names = data_args.text_column_names.split(",") # return a list
            # join together text columns into "sentence" column
            examples[text_column_names[0]] = ["[CLS]"+text_column_sample for text_column_sample in examples[text_column_names[0]]]
            examples["sentence"] = examples[text_column_names[0]]
            for column in text_column_names[1:]:
                for i in range(len(examples[column])):
                    #! generation is None
                    if examples[column][i] == None:
                        examples[column][i] = ""
                    #! convert list(instructions) to str
                    if isinstance(examples[column][i], list):
                        instructions = ""
                        for ins_dict in examples[column][i]:
                            for v in ins_dict.values():
                                instructions += v + "\n"
                        examples[column][i] = instructions
                    examples["sentence"][i] += data_args.text_column_delimiter + examples[column][i]
        # Tokenize the texts
        # AutoTokenizer return a dict e.g. {"input_ids": [], "attention_mask": []}
    ...
    if training_args.do_predict or data_args.test_file is not None:
        ...
        # remove label column if it exists (when do_predict below)
    ...
    if training_args.do_train:
        for index in random.sample(range(len(train_dataset)), 3): # random sample 3 examples from train_dataset
            logger.info(f"Sample {index} of the training set: {train_dataset[index]}.")
    
            

    #!: add metrics, don't use evaluate lib 
    #p.prediction, p.label_ids are both numpy.ndarray
    def compute_metrics(p: EvalPrediction):
        ...
        if is_regression:
            ...
            # change preds to torch.tensor
            # preds = np.array(preds, dtype='int32')
            preds = torch.tensor(preds)
            sigmoid = torch.nn.Sigmoid()
            preds = sigmoid(preds)
            preds = preds.cpu().detach().numpy()
            # for each logits in preds, if >0.5, then 1, else 0
            preds = np.array([np.where(p > 0.5, 1, 0) for p in preds], dtype='int32')
            p.label_ids = np.array(p.label_ids, dtype='int32')
            # use sklearn.metrics to compute metrics
            result["accuracy"] = accuracy_score(p.label_ids, preds) # Exact match ratio
            result["macro_f1"] = f1_score(p.label_ids, preds, average='macro')
            result["macro_precision"] = precision_score(p.label_ids, preds, average='macro')
            result["macro_recall"] = recall_score(p.label_ids, preds, average='macro')
            result["micro_f1"] = f1_score(p.label_ids, preds, average='micro')
            result["micro_precision"] = precision_score(p.label_ids, preds, average='micro')
            result["micro_recall"] = recall_score(p.label_ids, preds, average='micro')
        else:
            # preds = np.array(preds, dtype='int32')
    ...
    class CustomLossTrainer(Trainer):
            #! rewrite compute_loss function
        def compute_loss(self, model, inputs, return_outputs=False):
            outputs = model(**inputs)
            if is_regression:
                outputs["logits"] = torch.squeeze(outputs["logits"])
                #! normalize logits using log10 / ln
                # inputs['labels'] = torch.log10(inputs['labels'])
                inputs['labels'] = torch.log(inputs['labels'])
                loss_func = torch.nn.MSELoss()
                loss = loss_func(outputs['logits'], inputs['labels'].float())
    ...
    if training_args.do_train:
        ...
        train_result = trainer.train(resume_from_checkpoint=checkpoint) #? train_result type
        metrics = train_result.metrics #? what is metrics for train_result
        max_train_samples = (
    ...
    if training_args.do_predict:
        ...
        if is_regression:
            #! recover from log10 normalization
        ...
        model_args.model_name_or_path = model_args.model_name_or_path.replace("/", "_") # change name to save
        output_predict_file = os.path.join(training_args.output_dir, f"{model_args.model_name_or_path}_predict_results.txt") #can change
